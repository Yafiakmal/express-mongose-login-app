...
export async function generateAccessToken(payload: object) {
    if (!process.env.JWT_AT_SECRET) {
      ...
      const tokenPayload = {
        ...payload,
        iat: now,
        exp: now + parseInt(process.env.JWT_AT_SECRET_EXPIN ? process.env.JWT_AT_SECRET_EXPIN : "300"),// 5 menit
      };
...
export async function generateRefreshToken(payload: Record<string, any>) {
    ...
    const tokenPayload = {
      ...payload,
      iat: now,
      exp: now + parseInt(process.env.JWT_RT_SECRET_EXPIN ? process.env.JWT_RT_SECRET_EXPIN : "604800") // 7 hari
    };
